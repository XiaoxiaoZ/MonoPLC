# MonoPLC: 基于 Monoid 的无副作用 PLC 控制架构的数学模型与工业实践
...
纯粹的商业逻辑（“什么时候该开水阀”）与底层通信（“怎么发 MQTT”、“怎么等待网络延迟而不阻塞 Task”）被捏合在一起。如果网络堵塞，直接调用 `MQTT_Send` 会给整个毫秒级的实时 Task 带来问题。

---

## 2. 破局之道：引入 Monoid 代数结构

为了解决上述乱象，我们从抽象代数中借用了一个简单但强大的概念：**Monoid（幺半群）**。
...
这意味着：
- **封闭性 (Closure)**：一个 Effect（如水阀状态推送到手机）和另一个 Effect（如发日志），合起来依然也是一个完全相同的 `DUT_Effect_Monoid` 类型。不仅逻辑接口极度统一，还能实现大量的堆叠。处理 1 个操作和处理 100 个操作，在下游函数的入参定义上毫无区别：
  ```iecst
...
- **单位元 (Identity)**：如果这一毫秒的扫描周期内风平浪静，我们返回的并不是传统的“直接 RETURN 不做什么（Void/No-Op）”，或者是“返回一个 NULL 指针”，而是坚持返回一个“空的 Monoid”：
  ```iecst
  // 没有触发任何事件，返回一个空的 Monoid
  Effects_Out := FC_EmptyEffect(); 
  ```
  
  任何事物加上了 `FC_EmptyEffect()` 都依然等于它最初的自己。
...
- **结合律 (Associativity)**：这是应对多重并发和异步通信的关键。无论是 Task A 本地从 HMI 收到的直接电信号，还是 Task B 从网络 IoT 协议解析出的间接命令，只要它们都遵循这个 Monoid 规则，开发者就可以随意将它们像数学加法一样折叠：
  ...
  ```
  无论是在什么核心的哪个 Task 里、以何种顺序合并，最终传递给核心控制代码的，永远都是一个干净的数据类型。
...
为了实现 Monoid 模式，我们需要定义一组数据结构以及单位元。以下设计只是遵循 Monoid 规则的一个示例结构；只要满足代数定律，其他实现方式同样有效。
...
> 
> 但请注意：**这并不是唯一的合并方式。** 只要满足数学上的封闭性和结合律，在不同的应用场景中，你可以采用以下方式（例如）：
...
下面使用 Mermaid 语法展示了 MonoPLC 的事件循环机制以及外围副作用解耦的数据流向：
    ...
    subgraph Layer_Env ["1. 外部环境与物理设备层 (Physical & Environment)"]
        ...
        subgraph Layer_SideEffect ["2. 副作用异步处理层 (SideEffect_LOOP)"]
            ...
            subgraph Layer_Control ["3. 实时控制路由层 (Control_LOOP)"]
                ...
                subgraph Layer_Core ["4. 确定性业务核心层 (FB_SimpleLogic)"]
                    SafeCore((安全的核心控制逻辑 / 确定性逻辑块))
...
2. **实时控制循环 (`Control_LOOP`)**：
   ...
   - 然后，开启一个强大的 **事件路由分发器 (Event Dispatcher)**。这个阶段为你提供了一种架构自由：你既可以采用传统 PLC 的方式处理事件（直接硬件映射），也可以把它们当做副作用来处理。如果取出的事件是操作硬件，它可以立刻拉高拉低物理管脚；如果是发网络，它可以被扔进异步队列交给 `SideEffect_LOOP`。
   - **灵活性说明：事件路由并不是唯一的调度方式！**
     - 本示例中使用带有 `Event_Bus_Queue` 的单线程事件循环架构（类似 NodeJS 引擎）。
...
3. **核心控制代码层 (`FB_SimpleLogic`)**：
   - 这是业务 PLC 逻辑运算的大脑。在本项目示例中，我们在做一个简单的温度控制逻辑。
   - 它基于输入（可能结合内部状态）进行评估，思考的结果是产出一大堆**“想要做的副作用”结构体指令**（利用 `FC_CombineEffects` 收集封装）。
   - **重要架构声明：隔离才是本质！**
     - 架构的核心精髓在于将物理 I/O 和容易引发异常阻塞的异步任务，从确定性的控制主循环中彻底剥离出去。
     - 只要你的程序结构实现了“业务逻辑”与“导致网络拥堵或延时的副作用动作”隔离，让大脑成为一个专门评估当前输入副作用的确定性模块（Deterministic Logic Block），以 $f(State_t, InputEffect_t) \rightarrow (State_{t+1}, OutputEffect_t)$ 的数学映射形式，最终将决策意图纯粹地编码为一个 Effect Monoid，这套洋葱结构就能完美保护你的主程序免被外部环境波及。
     - **关于“纯函数 (Pure Function)”的注记**：虽然为了贴合工业界习惯，我们侧重使用“确定性模块 (Deterministic Logic Block)”，但其本质正是函数式编程中的**纯函数**理念。尽管此架构使严格意义上的纯函数（输出仅由当前输入决定）成为可能，但**在实际工程中一味盲目追求绝对的无状态往往是不必要的。**大多数工业控制仍然需要保持内部状态的持久化（如保留定时器计数、计次、或内部的状态机运转）。
...
在 MonoPLC 中，作为确定性核心的 `FB_SimpleLogic` 永远只耗费不到 1 毫秒完成状态转移计算并吐出了 `Target='mqtt'` 的 Effect 结构体。随后路由层秒速把它丢进带有指针锁的 `Async_Queue` 中跳出。网络发送库可以放在一个极低优先级（甚至 500ms Cycle）的副程式里慢慢轮询队列。
...
### 4.3 额外收益：“时间旅行”与重放调试
由于所有的外部刺激都被 A 阶段转换为了内存格式一致的 `Input_Monoid`，且所有的行为都被捕捉并记录在 `Total_Out_Monoid` 中。这就使得记录每一周期的 `[时间戳, 外部环境数据, Inputs_In]` 成为可能。如果发生重大停机事故，可以直接将这些日志导入至一个虚拟的 PLC 运行环境（Virtual Runtime），以等比例回放当时的精确场景。
- **重放成功的条件**：这种确定性的回放是一种非常强大的“额外能力”。只有当你的 `FB_SimpleLogic` 是作为一个 **纯函数（Pure Function）** 时才能被彻底保障。只要它不发生隐式的直接外部副作用调用，一模一样的初始状态，配上一模一样的时序输入，就一定会得出 100% 相同的结果，使得 Bug 被完美复现。

## 5. 总结

MonoPLC 探索了将“确定性逻辑块 (Deterministic Logic Block)”与“Monoid 代数结构”等数学强约束机制引入 PLC（可编程逻辑控制器）开发的可行性。我们在一定程度上缓解了工业控制软件中常见的逻辑纠缠与状态冲突问题。这种将外部物理环境交互与内部业务逻辑进行物理隔离的设计模式，为提高大型自动化控制代码的可确定性（Determinism）、可测试性以及系统的长期稳定性，提供了一种工程参考。
