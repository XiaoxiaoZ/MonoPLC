...
- **结合律 (Associativity)**：这是应对多重并发和异步通信的关键。无论是 Task A 本地从 HMI 收到的直接电信号，还是 Task B 从网络 IoT 协议解析出的间接命令，只要它们都遵循这个 Monoid 规则，开发者就可以随意将它们像数学加法一样折叠：
  ...
  ```
  无论是在什么核心的哪个 Task 里、以何种顺序合并，最终传递给核心控制代码的，永远都是一个清洗干净、合并规整的抽象“输入数据集合”。
...
3. **核心控制代码层 (`FB_SimpleLogic`)**：
   - 这是业务逻辑运算的大脑。在本项目示例中，为了在工业软硬件实体需求与架构数学严谨性之间取得最佳平衡，它被设计为一个**无副作用状态机 (Side-Effect-Free State Machine)**。**但是，本架构绝不限制你的内部设计模式。**
   - 你**不需要**受限于状态机。无论你使用简单的组合逻辑、无状态纯函数、复杂的顺控图 (SFC) 还是哪怕最传统的 IF-ELSE，唯一的铁律是：内部没有定时器，没有网络连接，也不能直接控制物理管脚。它只接收**当前的僵死数据**和一个引发激活的**单一输入事件**。
   - 它基于特定输入（可能结合内部状态）进行纯粹的数学评估，思考的结果是产出一大堆**“想要做的副作用”结构体指令**（利用 `FC_CombineEffects` 收集封装）。
   - ⚠️ **重要架构声明：隔离才是本质！**
     - 架构的核心精髓在于将物理 I/O 和容易引发异常阻塞的异步任务，从确定性的控制主循环中彻底剥离出去。
     - 只要你的程序结构实现了“业务逻辑”与“导致网络拥堵或延时的副作用动作”隔离，让大脑成为一个专门评估当前输入事件、执行确定性内部状态更新、最终将决策意图编码为 Effect Monoid 吐出去的确定性模块（Deterministic Logic Block），这套洋葱结构就能完美保护你的主程序免于外部环境的雪崩波及。
     - 💡 **关于“纯函数 (Pure Function)”的注记**：虽然为了贴合工业界习惯，我们在此处侧重使用了“确定性模块 (Deterministic Logic Block)”一词，但该模块的数学本质正是函数式编程中的**纯函数**理念。对于不需要维护内部状态的无状态逻辑，此架构完全可以用来实现严格意义上的纯函数（输出仅由当前输入决定，且不产生任何副作用）。MonoPLC 架构通过将一切副作用抽象为 Monoid 数据结构，为在传统 PLC 环境中真正落地纯函数概念提供了必要的基础设施。
...
MonoPLC 架构的真正核心与最终目的永远只有一个：**将“确定性计算逻辑”与“对外部世界产生副作用（Side Effect）的操作”彻底剥离。**

正是因为业务运算只产生作为中介数据包的 `DUT_Effect_Monoid`，而绝不去直接触模物理引脚或网络底层接口，所以**开发者拥有了对外部框架拥有随心所欲的改造自由**。

譬如说，为了追求极致的异步表现，开发者完全可以凭个人喜好，瞬间把 `Control_LOOP` 重构成一个类似于 **Node.js 引擎** 或 **现代 Actor 模型** 的单线程事件循环架构，而里面的确定性核心模块 `FB_SimpleLogic` 甚至连一行代码都不需要动：
...
3. **副作用递归反馈（Event Recursion）**：如果弹出的是提供给大脑的指令（如 `TICK`），就送给确定性逻辑块进行状态评估。计算出来的“输出副作用（Effects）”**绝不立刻执行**，而是反手**作为新事件，完全丢回 `Event_Bus_Queue` 的尾巴上排队**，等待循环引擎自己慢慢消化。
